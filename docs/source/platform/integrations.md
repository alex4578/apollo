...
Building tools to help you safely collaborate on the evolution of your GraphQL schema is one of our biggest focuses at Apollo. To make [schema change validation](/platform/schema-validation/) as easy to set up as possible, we've built an Apollo app for GitHub that provides status checks on pull requests when schema changes are proposed.
...
1. Send a [**daily snapshot**](#daily-reports) of the request rate, error rate, and performance latency of your GraphQL service.
